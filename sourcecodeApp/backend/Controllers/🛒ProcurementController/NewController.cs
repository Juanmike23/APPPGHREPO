/*
 * PGH-DOC
 * File: Controllers/🛒ProcurementController/NewController.cs
...
namespace PGH.Controllers.Procurement
{
    ...
    public class NewProcureController : ControllerBase
    {
        ...
        public NewProcureController(IConfiguration config, IMapper mapper, AppDbContext db, IWebHostEnvironment env)
        {
        ...
        [HttpGet]
        public async Task<ActionResult<IEnumerable<NewProcureReadDto>>> GetAll()
        {
            var list = await _db.ProcurementItems
                .Where(item => item.SourceType == ProcurementCanonicalHelper.SourceNew)
                .OrderByDescending(item => item.CreatedAt ?? item.UpdatedAt)
                .ThenByDescending(item => item.Id)
                .ToListAsync();
            var dtoList = list.Select(ProcurementCanonicalHelper.ToNewDto).ToList();
            ...
            return await ExecuteProcurementListRequestAsync(
                async ct =>
                {
                    ...
                    var query = _db.ProcurementItems
                        ...
                        .OrderByDescending(item => item.CreatedAt ?? item.UpdatedAt)
                        .ThenByDescending(item => item.Id)
                        .Select(item => new NewProcureReadDto
                        {
                            Id = item.Id,
                            No = item.No,
        ...
        [HttpPatch("{id}")]
public async Task<IActionResult> PatchNewProcure(long id, [FromBody] Dictionary<string, object> changes)
{
    if (changes == null || changes.Count == 0)
        return BadRequest("No changes provided.");

    var newprocure = await _db.ProcurementItems
        .FirstOrDefaultAsync(item => item.Id == id && item.SourceType == ProcurementCanonicalHelper.SourceNew);
    if (newprocure == null)
        return NotFound($"NewProcure with ID {id} not found.");
    ProcurementCanonicalHelper.ApplyPatchToItem(newprocure, changes);
    await ProcurementCanonicalHelper.ApplyProjectIdentityAsync(_db, newprocure);

    // Persist changes
    await _db.SaveChangesAsync();

    var updatedRow = ProcurementCanonicalHelper.ToNewDto(newprocure);

    return Ok(new
    {
        Message = "Row updated successfully",
        UpdatedAt = newprocure.UpdatedAt,
        row = updatedRow,
        newprocure = updatedRow
    });
}
        ...
        public async Task<IActionResult> Create([FromBody] NewProcure newprocure)
        {
            ...
            await _db.SaveChangesAsync();
            return Ok(ProcurementCanonicalHelper.ToNewDto(item));
        }
        ...
        public async Task<IActionResult> BulkDelete([FromBody] List<long> ids)
        {
            ...
            return await strategy.ExecuteAsync(async () =>
            {
                ...
                try
                {
                    ...
                    var deleteCanonicalRelationSql = $@"
        ...
        ";
            ...
            WHERE Id IN ({inClause}) AND SourceType = 'NEW'
        ";

                    await _db.Database.ExecuteSqlRawAsync(deleteCanonicalRelationSql, parameters);
                    await _db.Database.ExecuteSqlRawAsync(deleteStatusSql, parameters);
                    await _db.Database.ExecuteSqlRawAsync(deleteNewSql, parameters);
        ...
        [HttpGet("{id}")]
        public async Task<ActionResult<NewProcureReadDto>> GetById(long id)
        {
            ...
            var dto = ProcurementCanonicalHelper.ToNewDto(entity);
            return Ok(dto);
        }
        ...
        public async Task<IActionResult> GenerateProjectId(long id)
        {
            ...
            return Ok(new
            {
                ...
                id = newprocure.Id,
                project_id = newprocure.project_id
            });
        }
        ...
        // ✅ GET ChangeLog entries filtered by EntityId and field name (e.g. NoPKS)
        [HttpGet("changelog/{entityId}")]
        public async Task<IActionResult> GetChangeLogForEntity(
      long entityId,
      [FromQuery] string? field = null)
        {
            ...
            // Base query: procurement rows now log against the canonical ProcurementItem table.
            var query = _db.ChangeLog
                .Where(c =>
                    c.EntityId == entityId &&
                    (c.TableName == "ProcurementItem" || c.TableName == "NewProcure"));

            // 🎯 Filter by field name pattern if provided
            if (!string.IsNullOrEmpty(field))
            {
                // Match "Field 'NoPKS' changed from" to ensure accurate filtering
                query = query.Where(c =>
                    EF.Functions.Like(c.ChangeSummary, $"%Field '{field}'%"));
            }
            ...
            if (!logs.Any())
            {
                return NotFound(new
                {
                    Message = $"No ChangeLog entries found for EntityId {entityId}" +
            ...
            int yearToken)
        {
            var siblings = await _db.ProcurementItems
                ...
                .Where(item =>
                    item.Id != row.Id &&
                    item.SourceType == ProcurementCanonicalHelper.SourceNew &&
